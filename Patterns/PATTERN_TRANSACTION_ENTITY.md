...
draft.Set(new HexIdComponent { Coords = opened.Coords });     // identifying state from the opening command
draft.Set(new MyChoiceComponent { Value = default });         // session state, owned HERE from birth
draft.Set(new MyActionDraftTag());                            // stage tag: draft
...
{
    var picked = pulse.Get<MyChoicePickedEvent>().Value;
    _drafts.GetEntities()[0].Set(new MyChoiceComponent { Value = picked });
}

// 3. Commit — promote by tag swap; the entity already carries the full, live transaction state.
entity.Remove<MyActionDraftTag>();
entity.Set(new MyActionTag());
...
fact.Set(new MyResultComponent { Value = result });
fact.Set(new MyFactTag());                                    // the substrate's own table

var pulse = _world.CreateEntity();
pulse.Set(new MyFactCreatedEvent());                          // consumers reconcile from the fact table
