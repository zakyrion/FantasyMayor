...
stage is dead weight — commit directly on the confirming pulse (an event with values composed by the UI)
...
| Session state copied onto an event across the boundary (a full state copy on the opening event) | The copy goes stale against the entity; a re-stamp elsewhere papers over it | The event carries the command's values; session state is read live off the entity |
