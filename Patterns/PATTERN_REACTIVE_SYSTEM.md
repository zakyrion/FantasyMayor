...
public sealed class [Name]System : UpdatedSystem
{
    ...
    public [Name]System(EntityStore store)
        : base(store, EventArchetypes.Of<[Name]Event>(store))   // the pulse's archetype drives the system
    {
        _store = store;
        _rowsByKey = store.ComponentIndex<[Key]Component, [KeyValue]>();
        _rows = [Domain]Archetypes.[Table](store);
    }
...
(def reactive-rules
  {:on-pulse "reconcile, never delta"        ;; rebuild desired state from CURRENT store data and diff — idempotent: a 2nd pulse finds nothing to do; a missed pulse is repaired by the next
   :driven-by "EventArchetypes.Of<TheEvent>" ;; zero cost while no pulse exists; the event is payload-less (PATTERN_EVENT) — persistent truth lives in a world component / on an entity
