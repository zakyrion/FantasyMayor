...
EXACTLY one (ARCHITECTURE → Entities, tag-law). One approach.
...
```clojure
(def tag-rules
  {:add             "named in the table's archetype — Tags.Get<[Name]Tag>() inside its holder method"  ;; the row is BORN with it (ARCHITECTURE → Entities, archetype-law); tagging a live entity does not happen anywhere in this codebase — it would migrate the row out of its archetype
   ...
   :naming-prefix   :none                                   ;; namespace carries the domain; Table-Rule discriminators are the exception (ARCHITECTURE → Code shape, naming)
