...
| `event` | a struct implementing `IEventTag`, or named `…Event` / `…EventComponent` |
...
| `archetype` | a holder member `store.GetArchetype(ComponentTypes.Get<…>(), Tags.Get<…>())`, or the singleton manifest |
...
| `event_readers` | every readonly field (or constructor parameter kept in one) whose type is `EventReader<TEvent>` — what `reacts_to`, `polls` and `consumes` key on |
...
| `emits` | `EntityStorages.Events.Raise(new TEvent{…})` |
| `reacts_to` | a class holding an `EventReader<TEvent>` field with no `PerFrame` marker → that event type |
| `polls` | a class holding an `EventReader<TEvent>` field under the `PerFrame` marker → that event type |
| `consumes` | a non-system holder of an `EventReader<TEvent>` field — a game state, a subsystem, Boot — → that event type, with no system role |
...
- `tag_audit` — archetype without a main tag, several main tags, a main tag not written first, more than four label
  tags beside it, a main tag shared by two archetypes, an event type not implementing `IEventTag`, a label tag in a
  cross-archetype filter, a non-event filter with a tag count other than 1, a tag added during an entity's life,
  tags composed through `Tags.Add`. Every entry carries the `rule` it breaks.
...
| `tag/main-tag-unique` | one main tag shared by two archetypes, with no exception |
| `tag/event-tag` | an event type not implementing `IEventTag` |
...
| `event/raise` | `AddComponent` of an event type anywhere outside `EntityStorages.Events.Raise` |
| `event/suffix` | a second live type carrying the legacy `…EventComponent` suffix |
| `event/no-tag-on-persistent-row` | a component of an event type (implementing `IEventTag`) on an archetype in `World` or `Singletons` |
...
| `system/marker-required`, `system/marker-forbidden`, `system/marker-value` | an Update-loop class holding an `EventReader<TEvent>` field yet ticking every frame with no `PerFrame` marker; a marker on a class holding no `EventReader<TEvent>` field; a marker value other than `PerFrame` |
