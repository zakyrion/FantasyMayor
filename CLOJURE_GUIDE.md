...
| число | значення як є; під `:confidence` — int 0-100, наскільки ймовірно ВАРІАНТ є правильним рішенням (§14) | `600`, `0.5`, `:confidence 70` |
...
`:keyword` (+ namespaced), символ-vs-рядок (якір/проза), число-літерал (confidence 0-100),
`(cond)`, `(-> …)`, `(def subject {…})`, `(:label payload …)`-вузол, `!`-суфікс, `?`,
`:by-<джерело>`, `:keyword`-посилання на `:task`-id, `^:meta`-теги, констрейнт-ключі,
`:accept`, `;;`.
...
## 14. Deliverable-kind, confidence і провенанс (2026-08-29, з SDD 0.1.2/0.1.4)

Три нові жанри записів. Форми ті самі (§3-§4) — нові тільки ролі полів; канонічні правила
живуть у проєктному `CLAUDE.md` (`deliverable-kind`, `option-confidence`, `provenance`,
`attempted`, зона research).

**Питання — це теж інженерна задача, але з kind `:answer`.** Деліверабл = сама відповідь;
агент показує класифікацію inline і НЕ ескалює її у план чи зміни (ескалює тільки твоє
явне прохання):

```clojure
{:input "Яка структура папок правильна для нового домену?"
 :normalized {:task :ask-domain-layout
              :kind :answer
              :goal "правило + канонічна розкладка домену"
              :result "відповідь; жодних змін на диску"}}
```

**Кожен запропонований варіант несе confidence** — int 0-100, «наскільки ймовірно це
правильне РІШЕННЯ» (не істинність факту — ґрунт факту виражає `:verified-by`). Числа
пишуться у FLOW поруч із варіантом, щоб оцінку потім можна було звірити з реальністю:

```clojure
{:question "як спавнити вьюху району?"
 :options [{:option "реактивна система на built-event" :confidence 70}
           {:option "спавн усередині build-action"     :confidence 30}]}
```

**Записи з провенансом** — finding у FLOW завжди каже, ЯК його встановлено (`:verified-by`)
і КОЛИ (`:at`); покинутий підхід іде в Attempted з confidence, який він мав на момент
пропозиції:

```clojure
{:finding :turns-decremented-in-one-place
 :at "2026-08-28"
 :fact "лічильник ходу декрементує рівно одна система"
 :verified-by "прогнав turn-loop і дивився на компонент; не виведено з коду"
 :consequence "другий decrement-сайт = баг, не design choice"}

{:approach "декремент у UI-хендлері підтвердження"
 :confidence 40
 :dropped-because "хендлер не виконується на завантаженому сейві"
 :problems "2 ходи тихо губились на reload; знайдено тільки replay-ом сейва"
 :at "2026-08-28"}
```

**Deep-research запис** (`RESEARCH_TEMPLATE.md` → `Flows/RESEARCH_<TOPIC>.md`): здогадка
агента фіксується ДО пошуку — досить конкретна, щоб пошук міг її вбити; опція зважується
джерелами (виміряв > запустив > стверджує > знання агента):

```clojure
{:hunch "quadtree тут поб'є фіксовану сітку"
 :confidence 60
 :grounded-in "тільки знання агента — ще нічого не читано"
 :at "2026-08-28"
 :outcome :killed
 :deviation "джерела переформулювали питання у memory bandwidth, не lookup cost"}

{:option "chunked streaming з фіксованим LOD-кільцем"
 :applies-when "відкритий світ більший за пам'ять, single-player latency budget"
 :known-uses "кілька shipped open-world тайтлів"
 :evidence "один виміряний benchmark + два practitioner-звіти з тим самим результатом"
 :weakened-by "жодне джерело не міряло на нашій платформі"
 :confidence 70
 :cost-to-build "тижні"
 :cost-to-adopt "дні — плагін уже це вміє"
