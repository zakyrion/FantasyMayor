...
(def program
  {:ticket   "FM-13"
   ...
   :status   {:s0 :todo :s1 :todo :s2 :todo :s3 :todo :s4 :todo
...
(def porting-laws
  {:writes           "component writes ONLY via AddComponent — never through a ref (Friflo indexed components make ref-writes corrupting, not just illegal)"
   ...
   :structural       "snapshot-before-iterate stays the default idiom; CommandBuffer only where a snapshot is genuinely awkward"
