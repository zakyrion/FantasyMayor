...
related:
  ...
  - "[SYSTEMTEMPLATE](../../../../SYSTEMTEMPLATE.md)"
status: partial
code_refs:
  ...
  tags:             [DistrictOpenConditionTag, DistrictSingleOpenConditionTag]
...
- **A condition is an ENTITY, not a world component — deliberate departure from the config flow.** Only the
  ...
  concrete condition. The "config = world component, never an entity" rule of `CONFIGTEMPLATE.md` applies to
  the container only.
...
- **Orchestrator + per-type subsystem** (Template 4) instead of a single type-switch: each condition kind owns
...
**No consumer reads the condition entities yet** — the build-gate that evaluates them is not implemented.
Authored `.asset` (the container + per-condition assets) and the addressable key `DistrictOpenConditionsConfig`
are owned outside code.
