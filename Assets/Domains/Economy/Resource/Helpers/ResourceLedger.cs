...
namespace Domains.Economy.Resource.Helpers
{
    ...
    public static class ResourceLedger
    {
        ...
        public static void Credit(ReadOnlySpan<Entity> stacks, ReadOnlySpan<ResourceAmount> amounts)
        {
            foreach (var amount in amounts)
                Credit(stacks, amount);
        }

        public static void Credit(ReadOnlySpan<Entity> stacks, ResourceAmount amount)
        {
            if (amount.Amount <= 0)
                return;
            ...
            var current = stack.Get<ResourceComponent>();
            stack.Set(new ResourceComponent { Type = current.Type, Amount = current.Amount + amount.Amount });
        }
