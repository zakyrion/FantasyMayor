...
namespace Domains.Actions.Installer
{
    ...
    //  - BuildDistrictActionSystem / BuildDistrictCompletionSystem — the build-district Gameplay pair: the first
    //    (reactive) creates the in-progress entity on confirm, the second (per-frame) materialises the Economy
    //    District fact once its turn countdown hits 0. The countdown itself is ticked by BuildDistrictTurnTickSystem
    //    (a turn phase, above). Both Gameplay systems registered concrete so Boot injects them into the Gameplay
