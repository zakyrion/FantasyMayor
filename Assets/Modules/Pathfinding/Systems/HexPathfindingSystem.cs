...
namespace Modules.Pathfinding.Systems
{
    /// <summary>
    ///     DoD pathfinding system that performs BFS over the current set of hex entities in the ECS world.
    /// </summary>
    [UsedImplicitly]
    public sealed class HexPathfindingSystem : IHexPathfindingSystem, IDisposable
    {
        private readonly EntitySet _hexSet;

        public HexPathfindingSystem(World world)
        {
        ...
        /// <inheritdoc />
        public bool TryFindPath(HexCoord start, HexCoord end, Allocator allocator, out NativeList<HexCoord> path)
        {
            path = default;

            var capacity = math.max(1, _hexSet.Count);
        ...
        /// <summary>
        ///     Releases the cached ECS entity set used as the pathfinding domain source.
        /// </summary>
        public void Dispose()
        {
            _hexSet.Dispose();
        ...
        /// <summary>
        ///     Copies all currently existing hex coordinates from ECS into the native domain set.
        /// </summary>
        /// <param name="domain">Target native set to populate.</param>
        private void BuildDomain(ref NativeParallelHashSet<int2> domain)
        {
