...
(`design-mockups/FantasyMayor-HUD.html`, the `.tabs` row). Window of the `MainUI` module
(`ContextTabs/`). The three tabs are a **single-select `ui:Toggle` group** wired through **ECS events**: clicking
a tab raises a one-frame event, a system commits the new active tab into state and restyles, and a separate
system gates which tabs are interactable.
...
## Current state — markup authored; one Unity-side wiring step left
The components, event, enum, view, spawn subsystem and two systems exist and are wired in C#, **and the markup is
authored** in the shared `UI/MainUI` document (`Prefabs/HexInfoPanel.uxml` / `.uss`). There is no AP / economy
model — availability is a stub (all tabs always enabled).
...
## Markup contract
The view (`ContextTabsView`) resolves three `Toggle`s off the shared Main UI `UIDocument` by name. The markup
authors (already in `HexInfoPanel.uxml`):
- `ui:Toggle` elements named **`TabOverview`**, **`TabBuildings`**, **`TabActions`**, class `ctx-tab`;
...
2. **Selection** — `ContextTabSelectionSystem` (reactive, anchored on the event set) reads the current
   `ActiveContextTabComponent` and reconciles the view via `view.SetActive(active)`. Keeping the restyle in a
   system (not the MonoBehaviour) mirrors the `EndTurnView`/`EndTurnSystem` split; fail-loud if the state is
   missing or `Unknown`. Idempotent.
...
- **`ContextTabsView`** (`Views/`) — MonoBehaviour over the **shared** Main UI `UIDocument`. `[Inject]
  ...
  `SetActive(ContextTab)` (mirrors the active tab via `SetValueWithoutNotify` — `:checked` does the highlight) and
...
- **Tab content** — switching the active tab does not yet swap any context-body content; consumers of
  `ContextTabChangedEvent` / `ActiveContextTabComponent` come later.
