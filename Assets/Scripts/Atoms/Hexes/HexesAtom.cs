...
public class HexesAtom : IHexesAPI
{
    ...
    [Inject]
    private HexesAtom(HexMonoFactory hexMonoFactory, HexViewDataLayer hexDataLayer,
    ...
    UniTask<bool> IHexesAPI.CreateHex(HexViewData hexData, int detailLevel) =>
        _hexMonoFactory.SpawnHex(hexData, detailLevel);
    ...
    public List<HexViewData> SelectHexDataByLevel(HexViewData specificHexData, SelectionType selectionType)
    {
        ...
        while (queue.Count > 0)
        {
            ...
            if (needToAdd) selectedHexesData.Add(hexToCheck);
