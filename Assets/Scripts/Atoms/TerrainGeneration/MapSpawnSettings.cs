...
namespace HW.Data
{
    public static class MapSpawnSettings
    {
        ...
        public static int HexagonCount(int n) => 1 + 6 * (n * (n + 1) / 2);
        ...
        public static int HexCountInRing(int n) => 6 * n;
        ...
        {
            wave--;
            if (wave == 0) return new int2(0, 0); // Center hex
            ...
            for (var i = 0; i < steps; i++) position += directions[(side + 2) % 6]; // Move along the side
        ...
        {
            wave--;
            if (wave == 0) return (new int2(0, 0), new int2(0, 0)); // Center hex
            ...
            // Randomly choose a position along that side
            for (var i = 0; i < stepsOne; i++) positionOne += directions[(side + 2) % 6]; // Move along the side
            ...
            while (math.abs(stepsTwo - stepsOne) > 1) stepsTwo = _random.NextInt(0, wave);

            // Randomly choose a position along that side
            for (var i = 0; i < stepsTwo; i++) positionTwo += directions[(sideTwo + 2) % 6]; // Move along the side

            directions.Dispose();
            return (positionOne, positionTwo);
        }

        public static bool DropDice(int chance) => _random.NextInt(100) <= chance;

        public static bool DropDice(float chance) => _random.NextFloat() <= chance;
