...
AP tiles («Дії зараз» / «наст. хід»), and the global **End Turn** button pinned to the bottom. Window of the
`Presentation.UI` assembly (`EndTurn/`). The window keeps the `EndTurn` name even though it now owns the whole turn corner **and**
...
- **Hero CTA — the one place a filled-gold button is warranted.** Body gold `rgb(217,164,65)`, dark warm text
  `rgb(37,28,14)`, bold, letter-spacing ~2px, compact radius (sized to the HUD height budget — see
  `HEX_INFO_PANEL.md`). A lighter `border-top-color` fakes a
...
## Implementation (assembly `Presentation.UI`, window `EndTurn/`)
