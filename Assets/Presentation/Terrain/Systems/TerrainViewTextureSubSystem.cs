...
namespace Presentation.Terrain.Systems
{
    ...
    internal sealed class TerrainViewTextureSubSystem : ViewSubSystem
    {
        ...
        public override async UniTask Update(GameState state, CancellationToken cancellationToken)
        {
            ...
            var coastline = BuildCoastlineCoords(hexTypeMap);
            try
            {
        ...
        /// <summary>
        ///     Builds the base terrain-type lookup (<see cref="HexCoord" /> → domain <see cref="HexType" />)
        ///     by reading each hex's <see cref="HexTypeComponent" />. Coastline is NOT a hex type — it is
        ///     a view-only classification derived separately in <see cref="BuildCoastlineCoords" />.
        /// </summary>
        ...
        /// <summary>
        ///     Derives the coastline coord set — a Plain hex adjacent to at least one Water hex.
        ///     A view-only paint classification, not part of the domain <see cref="HexType" />.
        ///     Caller is responsible for disposing the returned set.
        /// </summary>
        private NativeParallelHashSet<HexCoord> BuildCoastlineCoords(
            NativeParallelHashMap<HexCoord, HexType> hexTypeMap)
        {
            var coastline = new NativeParallelHashSet<HexCoord>(hexTypeMap.Count(), Allocator.Persistent);

            foreach (var pair in hexTypeMap)
            {
                if (pair.Value != HexType.Plain)
                    continue;

                for (var d = 0; d < AxialMath.NeighborCount; d++)
                {
                    var neighbor = pair.Key + AxialMath.NeighborsPointyTop[d];
                    if (hexTypeMap.TryGetValue(neighbor, out var neighborType) && neighborType == HexType.Water)
                    {
                        coastline.Add(pair.Key);
                        break;
                    }
                }
            }
            ...
            float hexSize)
        {
            ...
            foreach (var coord in vertexGrid.Coords)
            {
                ...
                var baseColor = ComputeBlendedColor(vertex.Position.xz, vertex[0], slope, hexSize, slopeThreshold, config, hexTypeMap, coastline);
            ...
            NativeParallelHashSet<HexCoord> coastline)
        {
            ...
            for (var i = -1; i < AxialMath.NeighborCount; i++)
            {
                ...
                var c = PickColor(hexType, coastline.Contains(hex), slope, slopeThreshold, config);
                r += c.r * weight;
                g += c.g * weight;
                b += c.b * weight;
                totalWeight += weight;
            ...
            if (totalWeight <= 0f)
            {
                ...
                return PickColor(fallbackType, coastline.Contains(primaryHex), slope, slopeThreshold, config);
            }
