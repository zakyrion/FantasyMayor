...
Execute closes it. The lifecycle governs `:plan` and `:mutation` deliverables; a request
whose deliverable kind is `:answer` takes the answer lane instead (`deliverable-kind` /
`answer-contract` below) — no gate, no FLOW. Each phase is already backed by an existing gate:
